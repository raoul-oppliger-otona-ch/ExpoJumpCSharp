...
namespace ConsoleApp1
{
    ...
    class generationPages
    {
        ...
        public void testfile()
        {
            ...
            string sourceFile = @"C:\Users\raoul\source\repos\ConsoleApp1\ConsoleApp1\posters\belette\FOlder1\belette2.txt";
            string destinationFile = @"C:\Users\raoul\source\repos\xxx\xxx\site\posters\belette\FOlder1\test2.txt";

            // To copy a file or folder to a new location:
            //System.IO.File.Copy(sourceFile, destinationFile);

            // Effacer le fichier
            deleteMyFile(destinationFile);
            ...
            using (StreamReader sr = new StreamReader(sourceFile))
            {
                ...
                {
                    line = line.Replace("xyz", "Et oui Mais si !");
                }
                System.IO.File.WriteAllText(destinationFile, line);
        ...
        public void testIndex()
        {
            ...
            string masterFile = @"C:\Users\raoul\source\repos\ConsoleApp1\ConsoleApp1\_Layouts\master.html";
            string sourceFile = @"C:\Users\raoul\source\repos\ConsoleApp1\ConsoleApp1\index.html";
            string destinationFile = @"C:\Users\raoul\source\repos\xxx\xxx\site\index.html";

            deleteMyFile(destinationFile);

            StringBuilder contenu = new StringBuilder();
            StringBuilder modif = new StringBuilder();
            // On recopie le contenu
            contenu = mff.recopiage(masterFile);


            while (contenu.ToString().Contains("{{ content }}"))
            {
                sourceFile = @"C:\Users\raoul\source\repos\ConsoleApp1\ConsoleApp1\index.html";
                modif = mff.recopiage(sourceFile);
                contenu.Replace("{{ content }}", modif.ToString());
            }
            System.IO.File.WriteAllText(destinationFile, contenu.ToString());
        ...
        public void test33()
        {
            String sourcePosterFolder = ConfigurationSettings.AppSettings["rootFolder"] + "/posters";
            String destinationPosterFolder = ConfigurationSettings.AppSettings["siteFolder"] + "/posters";
            StringCollection lesFichiers = new StringCollection();
            manipFileFolder mesPosters = new manipFileFolder();
            //lesFichiers= mesPosters.listMyFiles(sourcePosterFolder, "txt");

            //Creation du repertoire poster

            Directory.CreateDirectory(destinationPosterFolder);

            //Copie de fichiers
            foreach (string name in lesFichiers)
            {
                //mesPosters.SimpleFileMove(name, destinationPosterFolder + "/" + name.Replace(sourcePosterFolder,""));
